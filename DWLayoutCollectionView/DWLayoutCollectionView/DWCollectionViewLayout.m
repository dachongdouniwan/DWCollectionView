...
-(UICollectionViewLayoutAttributes*)layoutAttributesForItemAtIndexPath:(NSIndexPath*)indexPath{
    ...
    if (1) {
        if (self.attributesArray.count!=0) {
            ...
            if (CGRectGetMaxX(lastAttributs.frame)+10==self.collectionView.bounds.size.width) {
                ///不在同一row
                ...
                CGFloat totleWidth = CGRectGetMaxX(lastFrame)+(10+width+10);
                ...
                if (totleWidth>=self.collectionView.bounds.size.width) {
                    ...
                    sameYWidth = sameYWidth + (20+(sameYArray.count-1)*10);
                    ///上一个row所剩下的宽度
